...
handle_privilege_drop() {
    ...
    if [[ $EUID -eq 0 ]]; then
        ...
        # Use sudo -E to preserve environment, and set PATH and PWD explicitly
        log "Switching to ansible user and re-executing..."
        exec sudo -E -u ansible env PATH="/home/ansible/.local/bin:$PATH" HOME="/home/ansible" PWD="/opt/network-upgrade" bash -c "cd /opt/network-upgrade && exec \"\$0\" \"\$@\"" "$0" "$@"
    fi
